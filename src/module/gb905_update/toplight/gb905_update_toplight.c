/****************************************Copyright (c)**************************************************                         
** File name:			gb905_update_toplight.c
** Descriptions:		 
**
**------------------------------------------------------------------------------------------------------
** Created by:		wenyu_xu
...
#define		GB905_UPDATE_TOPLIGHT_VENDOR_ID				0x06
#define		GB905_UPDATE_TOPLIGHT_HW_VERSION			0X01
#define		GB905_UPDATE_TOPLIGHT_SW_VERSION			0X0104

bool gb905_update_toplight_version_check(void)
{
	bool ret = true;
	int sw_version;
	...
	if(update_info.base_info.vendor_id != GB905_UPDATE_TOPLIGHT_VENDOR_ID)
	{
	...
	if(update_info.base_info.vendor_id != GB905_UPDATE_TOPLIGHT_HW_VERSION)
	{
	...
	if(sw_version <= GB905_UPDATE_TOPLIGHT_SW_VERSION)
	{
...
void gb905_update_toplight_start(void)
{
	bool ret;
	...
	if(!ret)
	{
		DbgWarn("update meter file is not exist!!\r\n");
		return;
	}

	if(stat(path,&file_stat) < 0)
	{
		DbgError("get update meter file size failed!! \r\n");
		return;
	}
