...
typedef struct {
	...
	unsigned char	voice_record_mode;					// 智能终端录音模式: =0X01,全程录音;=0X02,翻牌录音
	unsigned char	voice_record_period;				// 智能终端录音文件最大时长:（1-255）分钟
	unsigned char	lcd_heart_beat_time;				// 液晶（LCD）心跳时间间隔: (单位S 1-60)
	unsigned char	led_heart_beat_time;				// LED 心跳时间间隔: (单位S 1-60)
	
	unsigned short	time_idle_after_accoff;				// ACC OFF 后进入休眠模式的时间: 单位S

	unsigned char	video_protocol_mode;
